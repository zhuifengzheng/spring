...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
	...
	protected boolean isInfrastructureClass(Class<?> beanClass) {
		 ...
		 * todo isAssignableFrom()方法是从类继承的角度去判断，instanceof关键字是从实例继承的角度去判断。
		 * todo isAssignableFrom()方法是判断是否为某个类的父类，instanceof关键字是判断是否某个类的子类。
		 * todo 使用方法：
		 * todo 父类.class.isAssignableFrom(子类.class)
		 * todo 子类实例 instanceof 父类类型
		 *
		 * 假如当前正在创建的Bean的class
