...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			//2:获取告诉子类初始化Bean工厂 将bean加载到缓存中 spring springmvc是在该方法里面加载的bean定义
	...
	protected void prepareRefresh() {
		 ...
		 * 		//添加验证要求
		 * 		getEnvironment().getRequiredProperty("A");
		 *  }
		 * }
		 */
		initPropertySources();
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		/**
		 * springboot启动环境的时候，那么这这里bean工厂在springbooot里面已经被创建，所以调用GenericApplicationContext#refreshBeanFactory()刷新工厂。
		 * 普通的spring与springmvc的环境的时候，bean工厂还没有创建 需要通过AbstractRefreshableApplicationContext#refreshBeanFactory();创建
		 */
		refreshBeanFactory();
		//返回我们的bean工厂
...
//			EmbeddedValueResolver resolver = new EmbeddedValueResolver(this.getBeanFactory());
//			beanFactory.addEmbeddedValueResolver(re-> resolver.resolveStringValue(re));
			// 下面是java8之前的形式
...
//				}
//			});
			// todo java8函数式编程，参数可以是一个函数，形式为： 输入 -> 输出
